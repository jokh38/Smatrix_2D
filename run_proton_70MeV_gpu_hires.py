#!/usr/bin/env python3
"""GPU-optimized proton PDD simulation for 70 MeV protons with timing logs.
...
def main():
    print("=" * 70)
    print("PROTON PDD SIMULATION (GPU OPTIMIZED) - 70 MeV")
    print("=" * 70)
    ...
    if not use_gpu:
        # Use CPU fallback by creating equivalent operators
    ...
    # Create initial state using same method as CPU
    ...
    # Pre-compute stopping power for all energy bins
    E_centers = grid.E_centers
    E_edges = grid.E_edges
    stopping_power_grid = np.array([bethe_stopping_power_water(E, material, constants) for E in E_centers], dtype=np.float32)
    ...
    for step in range(max_steps):
        ...
        if use_gpu:
            # GPU simulation
            # Transfer state to GPU if needed
            if cp is not None:
                ...
                psi_gpu = state.psi
                E_edges_gpu = E_edges
                stopping_power_gpu = stopping_power_grid

            # Apply GPU transport step
            psi_new, weight_leaked_gpu, deposited_energy_gpu = gpu_transport.apply_step(
                psi=psi_gpu,
                E_grid=cp.asarray(E_centers, dtype=cp.float32),  # Energy bin centers
                sigma_theta=0.5 * np.sqrt(delta_z / E_init),  # Highland formula
                theta_beam=theta_init_rad,
                delta_s=delta_z,  # Step along z
                stopping_power=stopping_power_gpu,
                E_cutoff=E_cutoff,
                E_edges=E_edges_gpu,  # Energy bin edges for interpolation
            )

            # Update state
            if cp is not None:
                state.psi = cp.asnumpy(psi_new)
                state.weight_leaked += weight_leaked_gpu
                state.deposited_energy += cp.asnumpy(deposited_energy_gpu)
            else:
                # CPU fallback
    ...
    if use_gpu:
        ax.set_title(f'Proton PDD - {E_init} MeV in Water (GPU Optimized)')
    else:
        ax.set_title(f'Proton PDD - {E_init} MeV in Water (CPU Fallback)')
