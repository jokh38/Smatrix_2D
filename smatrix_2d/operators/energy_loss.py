...
class EnergyLossOperator:
    ...
    def apply(
        ...
        return psi_out, deposited_energy
            ...
            # CRITICAL FIX: Do NOT check E_cutoff before interpolation
            # The interpolation deposits weight to bins iE_target and iE_target+1
            # We must check E_cutoff for BOTH bins after deposition
            # Original bug: Check E_cutoff before interpolation, so particles
            # in the higher bin (which may be below cutoff) never get absorbed
            ...
            # Check if BOTH bins are above cutoff
            if self.grid.E_centers[iE_target] < E_cutoff and self.grid.E_centers[iE_target + 1] < E_cutoff:
                self._deposit_cutoff_energy(psi, deposited_energy, iE_src, E_new)
                continue
            
            # Check if lower bin is below cutoff
            if self.grid.E_centers[iE_target] < E_cutoff:
                # Particle moving from higher bin to lower (below cutoff)
                # Absorb at cutoff with full residual energy
                # CRITICAL: We must STILL deposit dose after absorbing
                self._deposit_cutoff_energy(psi, deposited_energy, iE_src, E_new)
                continue
            ...
            # Spec v7.2 Section 6.2: Coordinate-based fractional advection
            # This uses coordinate-based interpolation for energy transport
            # NOTE: Interpolation between bin centers creates discretization error
            # This is a known limitation of discrete energy grids
